...
class ParsingFFtt
{
	...
	private static $licensees_matchs_fftt_uri = "http://www.fftt.com/sportif/pclassement/php3/FFTTlp.php3";
    	private static $licensees_fftt_reqid_matchs = 400;
	...
	public function synchronizationAction()
	{
		...
		$licensees_match_list_parsing = array();
        	$licensees_match_list_parsing = $this->parseLicenseesMatchsFFTTCom( ParsingFFtt::$licensees_fftt_reqid_matchs, $licensees_list_parsing );
		/*
		echo "<pre>";
		...
		$date = new Datetime();
		$fp = fopen('parsing_fftt_' . $date->format('Y') . '_' . $date->format('m')  . '.csv', 'w+');

		foreach( $licensees_list_parsing as $licensee )
		{
			fputcsv($fp, array('N°licence', 'Nom', 'Prénom', 'Classement', 'Nombre de points', 'Catégorie'), ";" );
			fputcsv($fp, $licensee, ";");
		...
		echo "Cliquez ici pour t&eacute;l&eacute;charger le fichier : <a target='_blank' download='parsing_fftt_" . $date->format('Y') . "_" . $date->format('m')  . ".csv' href='http://www.cpfaizenay.com/parsing/parsing_fftt_" . $date->format('Y') . "_" . $date->format('m')  . ".csv'>parsing_fftt_" . $date->format('Y') . "_" . $date->format('m')  . ".csv</a>";
	}
                     ...
                     if( $key != 0 ) {
                         ...
                         foreach( $out[0] as $key2 => $elem ) {
                             ...
                             if( preg_match('#<a href=(.*)>#', $elem, $out_a_href ) ) {
                                 ...
                                 // Récupération du N° licence
                                 preg_match_all( "#precision%3D(.+)%26#isU", $out_a_href[1], $out2 );

                                 if( !empty( $out2[1][0] ) ) {
                                     $licensees_list_parsing[$currentIndex][ParsingFFtt::$licensees_list_parsing_format[$i]] = (int) $out2[1][0];
                                     $i++;
                                 }

                                 // Récupération du token FFTT permettant d'appeler l'url des matchs
                                 preg_match_all( "#cler=(.+)'>#isU", $out_a_href[1], $out3 );
                                 if( empty( $this->cler_request_fftt ) && !empty( $out3[1][0] )) {
                                     $this->cler_request_fftt = $out3[1][0];
                                 }
                             }

                             $licensees_list_parsing[$currentIndex][ParsingFFtt::$licensees_list_parsing_format[$i]] = trim( strip_tags( $elem ) );
                             $i++;
                         }
                         $currentIndex++;
                    }
    ...
    public function parseLicenseesMatchsFFTTCom( $reqid = "400", $licensee_list = array() ){
        $licensees_match_list_parsing = array();
        $moduloPosition = 25;
