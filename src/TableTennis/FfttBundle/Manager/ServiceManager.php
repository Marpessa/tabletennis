<?php
namespace TableTennis\FfttBundle\Manager;
...
class ServiceManager
{
    ...
    public function getClubsByDepartement($departement)
    {
        return $this->getCachedData("clubs{$departement}", 3600*24*30, function($service) use ($departement) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_club_dep2.php', array('dep' => $departement)), 'club');
        });
    }
    ...
    public function getClub($numero)
    {
        return $this->getCachedData("club_{$numero}", 3600*24*2, function($service) use ($numero) {
            return ServiceManager::getObject($service->getData('http://www.fftt.com/mobile/xml/xml_club_detail.php', array('club' => $numero)), 'club');
        });
    }
    ...
    public function getJoueur($licence)
    {
        $joueur = $this->getCachedData("joueur_{$licence}", 3600*24*1, function($service) use ($licence) {
            return ServiceManager::getObject($service->getData('http://allproxy4you2013.appspot.com/www.fftt.com/mobile/xml/xml_joueur.php', array('licence' => $licence, 'auto' => 1)), 'joueur');
        });
    ...
    public function getJoueurParties($licence)
    {
        return $this->getCachedData("joueur_parties_{$licence}", 3600*24*1, function($service) use ($licence) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_partie_mysql.php', array('licence' => $licence, 'auto' => 1)), 'partie');
        });
    }
    ...
    public function getJoueurPartiesSpid($licence)
    {
        return $this->getCachedData("joueur_spid_{$licence}", 3600*24*1, function($service) use ($licence) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_partie.php', array('numlic' => $licence)), 'resultat');
        });
    }
    ...
    public function getJoueursByName($nom, $prenom= '')
    {
        return $this->getCachedData("joueurs_{$nom}_{$prenom}", 3600*24*1, function($service) use ($nom, $prenom) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_liste_joueur.php', array('nom' => $nom, 'prenom' => $prenom)), 'joueur');
        });
    }
    ...
    public function getJoueursByClub($club)
    {
        return $this->getCachedData("joueursclub{$club}", 3600*24*1, function($service) use ($club) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_liste_joueur.php', array('club' => $club)), 'joueur');
        });
    }
    ...
    public function getEquipesByClub($club, $type = null)
    {
        ...
        $teams = $this->getCachedData("clubequipes_{$club}_{$type}", 3600*24*3, function($service) use ($club, $type) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_equipe.php', array('numclu' => $club, 'type' => $type)), 'equipe');
        });
    ...
    public function getPoules($division)
    {
        $poules = $this->getCachedData("poules_{$division}", 3600*24*30, function($service) use ($division) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_result_equ.php', array('action' => 'poule', 'D1' => $division)), 'poule');
        });
    ...
    public function getPouleClassement($division, $poule = null)
    {
        return $this->getCachedData("pouleclassement_{$division}_{$poule}", 3600*24*1, function($service) use ($division, $poule) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_result_equ.php', array('auto' => 1, 'action' => 'classement', 'D1' => $division, 'cx_poule' => $poule)), 'classement');
        });
    }
    ...
    public function getPouleRencontres($division, $poule = null)
    {
        return $this->getCachedData("poulerencontres_{$division}_{$poule}", 3600*24*1, function($service) use ($division, $poule) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_result_equ.php', array('auto' => 1, 'D1' => $division, 'cx_poule' => $poule)), 'tour');
        });
    }
    ...
    public function getOrganismes($type)
    {
        ...
        return $this->getCachedData("organismes_{$type}", 3600*24*30, function($service) use ($type) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_organisme.php', array('type' => $type)), 'organisme');
        });
    }
    ...
    public function getEpreuves($organisme, $type)
    {
        ...
        return $this->getCachedData("epreuves_{$organisme}_{$type}", 3600*24*30, function($service) use ($organisme, $type) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_epreuve.php', array('type' => $type, 'organisme' => $organisme)), 'epreuve');
        });
    }
    ...
    public function getDivisions($organisme, $epreuve)
    {
        return $this->getCachedData("divisions_{$organisme}_{$epreuve}", 3600*24*30, function($service) use ($organisme, $epreuve) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_division.php', array('organisme' => $organisme, 'epreuve' => $epreuve, )), 'division');
        });
    }
    ...
    public function getRencontre($link)
    {
        ...
        return $this->getCachedData("rencontre_".sha1($link), 3600*24*1, function($service) use ($params) {
            return ServiceManager::getObject($service->getData('http://www.fftt.com/mobile/xml/xml_chp_renc.php', $params), null);
        });
    }
    ...
    public function getLicencesByName($nom, $prenom= '')
    {
        return $this->getCachedData("licences_{$nom}_{$prenom}", 3600*24*2, function($service) use ($nom, $prenom) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_liste_joueur_o.php', array('nom' => strtoupper($nom), 'prenom' => ucfirst($prenom))), 'joueur');
        });
    }
    ...
    public function getLicencesByClub($club)
    {
        return $this->getCachedData("licencesclub_{$club}", 3600*24*2, function($service) use ($club) {
            return ServiceManager::getCollection($service->getData('http://www.fftt.com/mobile/xml/xml_liste_joueur_o.php', array('club' => $club)), 'joueur');
        });
    }
    ...
    public function getLicence($licence)
    {
        return $this->getCachedData("licence_{$licence}", 3600*24*1, function($service) use ($licence) {
            return ServiceManager::getObject($service->getData('http://www.fftt.com/mobile/xml/xml_licence.php', array('licence' => $licence)), 'licence');
        });
    }
    ...
    public function getData($url, $params = null)
    {
        ...
        $proxies[] = '190.198.81.154:8080';  
        /*$proxies[] = '189.210.203.131:8080'; // Some proxies require IP and port number
        $proxies[] = '201.155.49.184:8080';*/
        ...
        /*if ($this->ipsource) {
            curl_setopt($ch, CURLOPT_INTERFACE, $this->ipsource);
        }*/
